...
function QuickSale({ pushToast, onRecorded, store, lockKind }) {
  ...
  async function record() {
    ...
    D.cashSales.unshift({ id: "cs" + Date.now(), companyId: companyId || tagStore, clientId: activeClientId || null, date: D.today, client: clientLabel, type, kind, retDisp, item: label, total: +grand.toFixed(2), subtotal: r2(netSub), gst: r2(gst), pst: r2(pst), cogs: r2(costOut - costRestock), restockingFee: r2(fee), method: methodLabel, sales: ((window.__session && window.__session.userId) || "u_kev"), settle: settleDir, paid: r2(collected), owed: r2(onAccount), lines: useOut ? outLines.filter((l) => l.code && l.qty > 0).map((l) => ({ code: l.code, name: l.desc, qty: l.qty, price: l.price, cost: (itemByCode(l.code) || {}).cost || 0 })) : [] });
