...
function EmailSettings({ pushToast }) {
  ...
  function testConnection() {
    ...
    setTimeout(() => {
      const ok = draft.host && draft.user && +draft.port > 0;
      setConn((c) => ({ ...c, [active]: ok
        ? { state: "ok", msg: "220 " + draft.host + " ESMTP ready · " + draft.enc + " on port " + draft.port + " · auth OK" }
        : { state: "fail", msg: "Could not connect — " + (!draft.host ? "host is empty" : !draft.user ? "username is empty" : "invalid port") } }));
      pushToast && pushToast(ok ? "SMTP connection successful — " + draft.host : "SMTP connection failed");
    }, 1300);
  }
  ...
  return (
    <div className="set-grid">
      <Card title="Company information">
        <div className="meta-grid">
          <Field label="Business name"><input defaultValue={D.company.name} /></Field>
          <Field label="Phone"><input defaultValue={D.company.phone} /></Field>
          <Field label="Address line 1"><input defaultValue={D.company.addr1} /></Field>
          <Field label="Address line 2"><input defaultValue={D.company.addr2} /></Field>
          <Field label="GST number"><input defaultValue={D.company.gst} /></Field>
          <Field label="PST number"><input defaultValue={D.company.pst} /></Field>
        </div>
        <div className="exp-foot"><span /><Btn variant="primary" icon="check" onClick={() => pushToast && pushToast("Company details saved")}>Save</Btn></div>
      </Card>
...
function UsersPanel({ pushToast }) {
  ...
  function syncSalesperson(u) {
    let sp = D.salespeople.find((s) => s.id === u.id);
    const roleName = (roleById(u.role) || {}).name || "";
    if (sp) { sp.name = u.name; sp.initials = u.initials; sp.role = roleName; }
          ...
          {D.users.map((u) => (
            <tr key={u.id}>
              ...
              <td className="row-acts">
                ...
                <button className="icon-btn danger" title="Delete" onClick={() => setModal({ type: "delete", user: u })}><Icon name="trash" size={15} /></button>
              </td>
            </tr>
          ))}
        </tbody>
...
function UserFormModal({ user, onSave, onClose }) {
  ...
  function submit() {
    if (!valid) return;
    onSave({ name: name.trim(), email: email.trim(), role, password: password.trim(), active, initials: initialsOf(name) });
  }
